...
static unsigned char in[16] = {0x32, 0x43, 0xF6, 0xA8, 0x88, 0x5A, 0x30, 0x8D, 0x31, 0x31, 0x98, 0xA2, 0xE0, 0x37, 0x07, 0x34}, out[16], state[4][4];
static unsigned char RoundKey[240];
static unsigned char Key[32] = {0x3A, 0xE1, 0x15, 0x62, 0xA8, 0xF3, 0xC7, 0x1A, 0x2B, 0xF6, 0xDF, 0xA1, 0x50, 0x9B, 0xCA, 0xF1};
...
static void AddRoundKey(int round){
	...
	for(i=0;i<4;i++){
		for(j=0;j<4;j++){
			...
			printf("%02X",RoundKey[round * 4 * 4 + i * 4 + j]);
		}
		printf(" ");
	}
	printf("\n");
